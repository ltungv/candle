...
fn main() {
    ...
    println!("ddf={:?}", ddf.ravel());

    let dddf = grad(&two, |t| grad(&t, |t| grad(&t, |t| t.tanh())));
    println!("dddf={:?}", dddf.ravel());
    ...
    println!("prediction: {:?}", prediction.ravel());
    ...
    println!("w_grad: {:?}", wb_grads[0].ravel());
    println!("b_grad: {:?}", wb_grads[1].ravel());

    let new_w = &w - &w_grad;
    let new_b = &b - &b_grad;
    let new_prediction = predict(&new_w, &new_b, &inputs);
    let new_loss = loss(&new_w, &new_b, &inputs, &targets);
    println!("new_prediction: {:?}", new_prediction.ravel());
    println!("new_loss: {:?}", new_loss.ravel());
    ...
    println!("b_grad_numerical {:?}", b_grad_numerical.ravel());
    println!("b_grad_autodiff {:?}", b_grad.ravel());
}
